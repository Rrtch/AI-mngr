# WPPdarcy.py - Python TCP server for WhatsApp AI chatbot integration.
...
from LVL2 import *   # tu función interact(...) y P3
...
def handle_client(conn, addr):
    ...
    try:
        ...
        if body.strip().lower() in RESET_COMMANDS:
            ...
            print("📤 Enviado a Node (reset):", resp)
            return
        ...
        q = Queue()
        proc = Process(target=generate, args=(histories[user_id],body, q))
        ...
        if "error" in result:
            ...
            try:
                ...
                print("❌ Error enviando error a Node:", e)
            print("❌ Error en proceso hijo:", result["error"])
            return
        ...
        try:
            conn.sendall((json.dumps(resp) + "\n").encode())
            print("📤 Enviado a Node:", resp)
        except Exception as e:
            print("❌ Error enviando a Node:", e)
